...
def update_rule_confidence(project_root: str) -> dict:
    """
    Read FP log, compute 30-day rates, update rule_config.json.
    Returns a summary of changes made.
    """
    root = Path(project_root)
    config_path = root.parent / "config" / "rule_config.json"
    if not config_path.exists():
        config_path = Path(__file__).parent.parent / "config" / "rule_config.json"

    if not config_path.exists():
        return {"error": "rule_config.json not found"}

    config = json.loads(config_path.read_text())
    fp_log = root / "false_positives.jsonl"
    ...
    scan_log = root / "scan_history" / "scan_log.jsonl"

...
HIPAA_GUARD_ROOT="{hipaa_guard_root}"
...
python3 "$HIPAA_GUARD_ROOT/cli.py" scan $STAGED
...
def install(repo_root: str | None = None) -> None:
    ...
    # Find HIPAA-Guard root (directory of this script's parent)
    hipaa_guard_root = Path(__file__).parent.parent.resolve()
    ...
    if hook_path.exists():
        ...
        with hook_path.open("a") as f:
            ...
            f.write(_HOOK_SCRIPT.format(hipaa_guard_root=hipaa_guard_root))
        print("✅ HIPAA-Guard appended to existing pre-commit hook.")
    else:
        hook_path.write_text(_HOOK_SCRIPT.format(hipaa_guard_root=hipaa_guard_root))

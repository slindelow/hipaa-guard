...
from state_manager import ScanState, Severity


def record_scan(state: ScanState, project_root: str) -> None:
    """Append scan summary to scan_history/ after every completed scan."""
    history_dir = Path(project_root) / "scan_history"
...
def detect_drift(project_root: str) -> dict | None:
    """
...
def generate_weekly_report(project_root: str, api_key: str | None = None) -> str:
    """
    Generate a weekly compliance posture report.
    Returns the report as a markdown string.
    """
    history = _load_history(project_root, days=30)
    ...
    # FP rate
    fp_log = Path(project_root) / "false_positives.jsonl"
    fp_count = 0
    if fp_log.exists():
    ...
    # Drift check
    drift = detect_drift(project_root)
    if drift:
        lines += [
            "",
    ...
    # Save report
    report_path = Path(project_root) / ".tmp" / "weekly_report.md"
...
def _load_history(project_root: str, days: int = 30) -> list[dict]:
    log_file = Path(project_root) / "scan_history" / "scan_log.jsonl"

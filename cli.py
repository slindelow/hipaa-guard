...
from state_manager import new_session, ScanTrigger
from scanner import scan_files


def _load_config() -> dict:
    cfg_path = Path(__file__).parent / "hipaa-guard.config.json"
    if cfg_path.exists():
...
def _load_rule_config() -> dict:
    cfg_path = Path(__file__).parent / "config" / "rule_config.json"
    if cfg_path.exists():
        return json.loads(cfg_path.read_text())
    return {}


def _collect_files(paths: list[str], config: dict) -> list[str]:
...
def cmd_scan(args, config: dict, rule_config: dict) -> int:
    ...
    # Separate high-confidence from needs-analyst
    ...
    # Run full pipeline if Anthropic key available
    ...
    # Exit code: non-zero if blocking findings and not in onboarding mode
...
def cmd_fp(args) -> int:
    ...
    import datetime
    fp_log = Path(__file__).parent / "false_positives.jsonl"
    entry = {
...
def cmd_fn(args) -> int:
    ...
    import datetime
    fn_log = Path(__file__).parent / "false_negatives.jsonl"
    entry = {
        "file": args.file,
        "line": args.line,
...
def cmd_audit(args, config: dict, rule_config: dict) -> int:
    ...
    # Also run BAA check
    try:
        from baa_checker import check_baa
    ...
    try:
        from compliance_doc_generator import generate
...
def cmd_install_hook(args) -> int:
    ...
    sys.path.insert(0, str(Path(__file__).parent / "tools"))
    try:
        from git_hook_installer import install
        install()
        return 0
...
def cmd_baa(args, config: dict) -> int:
    ...
    repo_root = Path.cwd()
    try:
        from baa_checker import check_baa, print_baa_report
        results = check_baa(str(repo_root))
        print_baa_report(results)
        return 0
    except ImportError:
        print("BAA checker not yet available.")
        return 1
